...
public class EBulkProcessorListener implements BulkProcessor.Listener {
    ...
    private void syncLog(List<ESVersionLogBean> logList) {
        if (!CollectionUtils.isEmpty(logList)) {
            this.getESVersionService().insertLogList(logList);
        }
    }

    private void syncVersion(List<ESVersionBean> list) {
        if (!CollectionUtils.isEmpty(list)) {
            this.getESVersionService().replaceList(list);
        }
    }

    private ESVersionService getESVersionService() {
        return (ESVersionService) ServiceLocale.findService(ESVersionService.class);
    }
}
